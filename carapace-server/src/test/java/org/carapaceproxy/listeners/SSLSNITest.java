...
public class SSLSNITest {
    ...
    public void testChooseCertificate() throws Exception {
        ...
        try (HttpProxyServer server = new HttpProxyServer(mapper, tmpDir.getRoot());) {
            ...
            assertNull(Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), "no", "no-default"));

            assertEquals("*.qatest.pexample.it", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), "test2.qatest.pexample.it", "no-default").getId());
            // client requests SNI, bad default in listener
            assertEquals("other", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), "other", "no-default").getId());

            assertEquals("www.example.com", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), "unkn-other", "www.example.com").getId());
            // client without SNI
            assertEquals("www.example.com", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), null, "www.example.com").getId());
            // exact match
            assertEquals("www.example.com", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), "www.example.com", "no-default").getId());
            // wildcard
            assertEquals("*.example.com", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), "test.example.com", "no-default").getId());
            // san
            assertEquals("*.example.com", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), "example.com", "no-default").getId());
            assertEquals("*.example.com", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), "test.example2.com", "no-default").getId());
            ...
            assertEquals("*.test.example.com", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), "pippo.test.example.com", "no-default").getId());
            // san
            assertEquals("*.example.com", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), "example.com", "no-default").getId());
            assertEquals("*.example.com", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), "test.example2.com", "no-default").getId());
        }
        ...
        try (HttpProxyServer server = new HttpProxyServer(mapper, tmpDir.getRoot());) {
            ...
            assertEquals("*", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), null, "www.example.com").getId());
            assertEquals("*", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), "www.example.com", null).getId());
            assertEquals("*", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), null, null).getId());
            assertEquals("*", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), "", null).getId());
            assertEquals("*", Listeners.chooseCertificate(server.getListeners().getCurrentConfiguration(), null, "").getId());
        }
    }
